...
namespace RzrSite.DAL.Repositories
{
  public class DbFileRepo : IDbFileRepo
  {
	...
	public IEnumerable<IStrippedDbFile> GetAll()
	{
	  ...
	  var strippedFiles = files.ToList().Select((source, dest) => _mapper.Map<StrippedDbFile>(source));
	...
	{
	  var file = GetFull(id);
	  return _mapper.Map<StrippedDbFile>(file);
	}

	/// <summary>
	/// <inheritdoc/>
	/// </summary>
	public IStrippedDbFile Get(string path)
	{
	  var file = GetFull(path);
	  return _mapper.Map<StrippedDbFile>(file);
	}
	...
	/// <inheritdoc/>
	/// </summary>
	public IDbFile GetFull(string path)
	{
